...
- **Class-staff writes are globally serialized within one MCP server process.** A 2026-08-18 parallel batch caused associations on unrelated sections to disappear, and the confirmed same-section stale-read/full-replace race is reproducible with fake records. `AppContext.class_staff_write_lock` therefore covers the complete read-modify-write in both `add_class_staff` and `remove_class_staff`. MCP instructions and both tool descriptions also tell agents never to parallelize these calls, because separate server processes are not coordinated and the cross-section ParentSquare behavior remains unexplained. Agents should still fresh-read with `get_class` after each write and re-check involved users' full associations after multi-section changes.
